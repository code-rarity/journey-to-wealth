...
class Journey_To_Wealth_Public {
    ...
    public function ajax_fetch_analyzer_data() {
        ...
        if (!empty($overview['Industry']) && !empty($overview['Sector'])) {
            $this->store_and_map_discovered_industry($overview['Industry'], $overview['Sector']);
        }
    ...
    private function get_valuation_results($overview, $income_statement, $balance_sheet, $cash_flow, $treasury_yield, $latest_price, $daily_data) {
        ...
        $beta_details = $this->calculate_levered_beta($overview['Industry'], $balance_sheet, $overview['MarketCapitalization'], $tax_rate_decimal);
        $levered_beta = $beta_details['levered_beta'];

        $industry = strtoupper($overview['Industry']);
        $sector = strtoupper($overview['Sector']);

        $is_reit = preg_match('/R\.?E\.?I\.?T\.?|REAL ESTATE/i', $industry);
        $is_bank_or_insurance = preg_match('/BANK|INSURANCE/i', $industry);
        $is_financial = preg_match('/FINANCIAL/i', $sector) || $is_reit || $is_bank_or_insurance;

        if ($is_financial) {
            if ($is_reit) {
                $affo_model = new Journey_To_Wealth_AFFO_Model($erp_decimal, $levered_beta);
                $result = $affo_model->calculate($overview, $cash_flow, $treasury_yield, $latest_price, $beta_details);
                if (!is_wp_error($result)) {
                    $valuation_data['AFFO Model'] = $result;
                }
            } elseif ($is_bank_or_insurance) {
            ...
            // Fallback to DDM for financials if primary model fails
            ...
            // Non-financials, default to 2-Stage DCF
    ...
    private function store_and_map_discovered_industry($industry_name, $sector_name) {
        if (empty($industry_name) || empty($sector_name)) return;
    
        $discovered = get_option('jtw_discovered_av_industries', []);
        
        if (!in_array($industry_name, $discovered)) {
            $discovered[] = $industry_name;
            update_option('jtw_discovered_av_industries', $discovered);
            $this->auto_map_new_industry($industry_name, $sector_name);
        }
    }

    private function auto_map_new_industry($av_industry, $av_sector) {
        global $wpdb;
        $mapping_table = $wpdb->prefix . 'jtw_industry_mappings';
        $beta_table = $wpdb->prefix . 'jtw_industry_betas';

        $existing_mapping = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $mapping_table WHERE av_industry = %s", $av_industry));
        ...
        $damodaran_industries = $wpdb->get_col("SELECT industry_name FROM $beta_table");
        ...
        foreach ($damodaran_industries as $dam_industry) {
            $dam_industry_words = preg_split('/[\s,\/-]+/', strtolower($dam_industry));
            ...
            if ($matches > 0) {
                foreach ($industry_groups as $group_name => $keywords) {
                    foreach ($keywords as $keyword) {
                        if (stripos($dam_industry, $keyword) !== false) {
        ...
        $damodaran_industries_to_map = $wpdb->get_col("SELECT industry_name FROM $beta_table WHERE " . implode(' OR ', $where_clauses));

        if (!empty($damodaran_industries_to_map)) {
            foreach ($damodaran_industries_to_map as $damodaran_industry) {
                $wpdb->insert( $mapping_table, [ 'av_industry' => $av_industry, 'damodaran_industry' => $damodaran_industry ], ['%s', '%s'] );
            }
        }
    }

    private function calculate_levered_beta($av_industry, $balance_sheet, $market_cap, $tax_rate) {
        global $wpdb;
        ...
        $mapping_table = $wpdb->prefix . 'jtw_industry_mappings';
        $damodaran_industries = $wpdb->get_col($wpdb->prepare(
            "SELECT damodaran_industry FROM $mapping_table WHERE av_industry = %s",
            $av_industry
        ));
        ...
        $debug_data['mapped_damodaran_industries'] = $damodaran_industries;
    
        $beta_table = $wpdb->prefix . 'jtw_industry_betas';
        
        $unlevered_betas = [];
        foreach ($damodaran_industries as $industry) {
            $beta = $wpdb->get_var($wpdb->prepare(
                "SELECT unlevered_beta FROM $beta_table WHERE industry_name = %s",
                $industry
            ));
            if (is_numeric($beta)) {
                $unlevered_betas[] = (float) $beta;
            }
        }
        ...
        if ($market_cap > 0) {
            ...
            $levered_beta = $average_unlevered_beta * (1 + (1 - $tax_rate) * $debt_to_equity);
    ...
    private function build_peg_pegy_ratios_section_html($metrics, $stock_price, $eps) {
        $peg_data = $metrics['pegRatio'] ?? []; $pegy_data = $metrics['pegyRatio'] ?? [];
    ...
    private function build_intrinsic_valuation_section_html($valuation_data, $summary, $details) {
        ...
        if ($summary['fair_value'] > 0) {
            $output .= '<div class="jtw-valuation-chart-container" id="jtw-valuation-chart-container" data-current-price="' . esc_attr($summary['current_price']) . '" data-fair-value="' . esc_attr($summary['fair_value']) . '" data-percentage-diff="' . esc_attr($summary['percentage_diff']) . '"><canvas id="jtw-valuation-chart"></canvas></div>';
        } else {
    ...
    private function build_valuation_assumptions_modal_html($data, $details) {
        ...
        $output .= '<p>Below are the data sources, inputs and calculation used to determine the intrinsic value for ' . esc_html($ticker) . '.</p>';
        
        // **FIXED** Call the correct builder function for each model
        switch($model_name) {
            case 'DCF Model':
    ...
    private function build_dcf_modal_content($data, $details) {
        $beta_details = $data['discount_rate_calc']['beta_details'] ?? [];
        $output = '<h4>Discount Rate Calculation</h4><table class="jtw-modal-table"><thead><tr><th>Data Point</th><th>Calculation / Source</th><th>Result</th></tr></thead><tbody>';
        $output .= '<tr><td>Risk-Free Rate</td><td>' . esc_html($data['discount_rate_calc']['risk_free_rate_source']) . '</td><td>' . number_format($data['discount_rate_calc']['risk_free_rate'] * 100, 1) . '%</td></tr>';
        $output .= '<tr><td>Equity Risk Premium</td><td>' . esc_html($data['discount_rate_calc']['erp_source']) . '</td><td>' . number_format($data['discount_rate_calc']['equity_risk_premium'] * 100, 1) . '%</td></tr>';
        $output .= '<tr><td><strong>Levered Beta</strong></td><td><strong>' . esc_html($data['discount_rate_calc']['beta_source']) . '</strong></td><td><strong>' . number_format($data['discount_rate_calc']['beta'], 2) . '</strong></td></tr>';
        $output .= '<tr><td>Discount Rate/ Cost of Equity</td><td>' . wp_kses_post($data['discount_rate_calc']['cost_of_equity_calc']) . '</td><td>' . number_format($data['inputs']['cost_of_equity'] * 100, 1) . '%</td></tr>';
        $output .= '</tbody></table>';

        if (!empty($beta_details) && $beta_details['beta_source'] === 'Calculated from Industry Beta') {
            $output .= '<h4>Levered Beta Calculation</h4>';
            $output .= '<table class="jtw-modal-table"><tbody>';
            $output .= '<tr><td>Alpha Vantage Industry</td><td colspan="2">' . esc_html($details['industry']) . '</td></tr>';
            $output .= '<tr><td>Mapped Damodaran Industries</td><td colspan="2">' . esc_html(implode(', ', $beta_details['mapped_damodaran_industries'])) . '</td></tr>';
            $output .= '<tr><td>Average Unlevered Beta</td><td colspan="2">' . number_format($beta_details['unlevered_beta_avg'], 4) . '</td></tr>';
            $output .= '<tr><td>Tax Rate</td><td colspan="2">' . number_format($beta_details['tax_rate'] * 100, 1) . '%</td></tr>';
            $output .= '<tr><td>Debt/Equity Ratio</td><td colspan="2">' . number_format($beta_details['debt_to_equity'], 4) . '</td></tr>';
            $output .= '<tr><td><strong>Relevered Beta</strong></td><td colspan="2"><strong>' . number_format($beta_details['levered_beta'], 4) . '</strong></td></tr>';
            $output .= '</tbody></table>';
        }

        $output .= '<h4>Discounted Cash Flow Calculation</h4><table class="jtw-modal-table"><thead><tr><th>Year</th><th>Levered FCF (USD, Millions)</th><th>Present Value Discounted (@' . number_format($data['inputs']['cost_of_equity'] * 100, 1) . '%)</th></tr></thead><tbody>';
        foreach ($data['projection_table'] as $row) { $output .= '<tr><td>' . esc_html($row['year']) . '</td><td>$' . number_format($row['fcfe'] / 1e6, 0) . 'm</td><td>$' . number_format($row['pv_fcfe'] / 1e6, 0) . 'm</td></tr>'; }
        $output .= '<tr><td colspan="2"><strong>Present value of next 10 years cash flows</strong></td><td><strong>$' . number_format($data['sum_of_pv_fcfs'] / 1e6, 0) . 'm</strong></td></tr>';
        $output .= '</tbody></table>';

        $output .= '<h4>Final Valuation</h4><table class="jtw-modal-table"><thead><tr><th>Metric</th><th>Calculation</th><th>Result</th></tr></thead><tbody>';
        $output .= '<tr><td><strong>' . esc_html($data['terminal_value_calc']['label']) . '</strong></td><td>' . wp_kses_post($data['terminal_value_calc']['calc']) . '</td><td>' . esc_html($data['terminal_value_calc']['result']) . '</td></tr>';
        $output .= '<tr><td><strong>' . esc_html($data['pv_terminal_value_calc']['label']) . '</strong></td><td>' . wp_kses_post($data['pv_terminal_value_calc']['calc']) . '</td><td>' . esc_html($data['pv_terminal_value_calc']['result']) . '</td></tr>';
        $output .= '<tr><td><strong>' . esc_html($data['equity_value_calc']['label']) . '</strong></td><td>' . wp_kses_post($data['equity_value_calc']['calc']) . '</td><td>' . esc_html($data['equity_value_calc']['result']) . '</td></tr>';
        $output .= '<tr><td><strong>' . esc_html($data['value_per_share_calc']['label']) . '</strong></td><td>' . wp_kses_post($data['value_per_share_calc']['calc']) . '</td><td>' . esc_html($data['value_per_share_calc']['result']) . '</td></tr>';
        $output .= '<tr><td><strong>' . esc_html($data['discount_calc']['label']) . '</strong></td><td>' . wp_kses_post($data['discount_calc']['calc']) . '</td><td>' . esc_html($data['discount_calc']['result']) . '</td></tr>';
        $output .= '</tbody></table>';
        return $output;
    }
    ...
    private function build_ddm_modal_content($data, $details) {
        ...
        $output .= '<tr><td>Discount Rate (Cost of Equity)</td><td>' . number_format($data['discount_rate'] * 100, 2) . '%</td></tr>';
        ...
        $output .= '<tr><td><strong>Intrinsic Value</strong></td><td><strong>$' . number_format($data['intrinsic_value'], 2) . '</strong></td></tr>';
        $output .= '</tbody></table>';
        return $output;
    }

    private function build_affo_modal_content($data, $details) {
        $output = '<h4>Key Inputs</h4><table class="jtw-modal-table"><tbody>';
        $output .= '<tr><td>Base AFFO</td><td>$' . number_format($data['base_affo']) . '</td></tr>';
        $output .= '<tr><td>Initial Growth Rate</td><td>' . number_format($data['initial_growth_rate'] * 100, 2) . '%</td></tr>';
        $output .= '<tr><td>Terminal Growth Rate</td><td>' . number_format($data['terminal_growth_rate'] * 100, 2) . '%</td></tr>';
        $output .= '<tr><td>Discount Rate (Cost of Equity)</td><td>' . number_format($data['cost_of_equity'] * 100, 2) . '%</td></tr>';
        $output .= '</tbody></table>';

        $output .= '<h4>AFFO Projection</h4><table class="jtw-modal-table"><thead><tr><th>Year</th><th>Projected AFFO</th><th>PV of AFFO</th></tr></thead><tbody>';
        foreach($data['projection_table'] as $row) {
            $output .= '<tr><td>' . $row['year'] . '</td><td>$' . number_format($row['affo']) . '</td><td>$' . number_format($row['pv_affo']) . '</td></tr>';
        }
        $output .= '<tr><td colspan="2"><strong>Sum of PV of AFFO</strong></td><td><strong>$' . number_format($data['sum_of_pv_affos']) . '</strong></td></tr>';
        $output .= '</tbody></table>';

        $output .= '<h4>Final Valuation</h4><table class="jtw-modal-table"><tbody>';
        $output .= '<tr><td>Terminal Value</td><td>$' . number_format($data['terminal_value']) . '</td></tr>';
        $output .= '<tr><td>PV of Terminal Value</td><td>$' . number_format($data['pv_of_terminal_value']) . '</td></tr>';
        $output .= '<tr><td>Total Equity Value</td><td>$' . number_format($data['total_equity_value']) . '</td></tr>';
        $output .= '<tr><td>Shares Outstanding</td><td>' . number_format($data['shares_outstanding']) . '</td></tr>';
        $output .= '<tr><td><strong>Intrinsic Value Per Share</strong></td><td><strong>$' . number_format($data['intrinsic_value_per_share'], 2) . '</strong></td></tr>';
        $output .= '</tbody></table>';
        return $output;
    }
    ...
    private function build_excess_return_modal_content($data, $details) {
        $output = '<h4>Key Inputs</h4><table class="jtw-modal-table"><tbody>';
        $output .= '<tr><td>Current Book Value</td><td>$' . number_format($data['current_book_value']) . '</td></tr>';
        $output .= '<tr><td>Return on Equity (ROE)</td><td>' . number_format($data['roe'] * 100, 2) . '%</td></tr>';
        $output .= '<tr><td>Equity Growth Rate</td><td>' . number_format($data['equity_growth_rate'] * 100, 2) . '%</td></tr>';
        $output .= '<tr><td>Discount Rate (Cost of Equity)</td><td>' . number_format($data['cost_of_equity'] * 100, 2) . '%</td></tr>';
        $output .= '<tr><td>Terminal Growth Rate</td><td>' . number_format($data['terminal_growth_rate'] * 100, 2) . '%</td></tr>';
        $output .= '</tbody></table>';

        $output .= '<h4>Excess Return Projection</h4><table class="jtw-modal-table"><thead><tr><th>Year</th><th>Book Value</th><th>Excess Return</th><th>PV of Excess Return</th></tr></thead><tbody>';
        foreach($data['projection_table'] as $row) {
            $output .= '<tr><td>' . $row['year'] . '</td><td>$' . number_format($row['book_value']) . '</td><td>$' . number_format($row['excess_return']) . '</td><td>$' . number_format($row['pv_excess_return']) . '</td></tr>';
        }
        $output .= '<tr><td colspan="3"><strong>Sum of PV of Excess Returns</strong></td><td><strong>$' . number_format($data['sum_pv_excess_returns']) . '</strong></td></tr>';
        $output .= '</tbody></table>';
