 ...
 * Aligned with the single-stage model used by Simply Wall St.
 *
...
class Journey_To_Wealth_Excess_Return_Model {
    ...
    public function calculate($overview, $income_statement, $balance_sheet, $treasury_yield, $latest_price, $beta_details) {
        ...
        if ($this->cost_of_equity <= $this->terminal_growth_rate) {
            return new WP_Error('erm_k_not_greater_than_g', __('ERM: Cost of Equity must be greater than terminal growth rate.', 'journey-to-wealth'));
        }

        $excess_return_amount = ($roe - $this->cost_of_equity) * $current_book_value_equity;
        $terminal_value_of_excess_returns = $excess_return_amount / ($this->cost_of_equity - $this->terminal_growth_rate);
        
        $total_equity_value = $current_book_value_equity + $terminal_value_of_excess_returns;
        ...
        return [
            ...
            'calculation_breakdown' => [
                ...
                'current_book_value' => $current_book_value_equity,
                'excess_return_amount' => $excess_return_amount,
                'terminal_value' => $terminal_value_of_excess_returns,
